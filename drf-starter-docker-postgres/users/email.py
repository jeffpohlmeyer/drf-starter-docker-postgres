...
class CustomBaseEmail(EmailMultiAlternatives, ContextMixin):
    ...
    def __init__(self, request=None, context=None, template_name=None,
                 template_name_txt=None, *args, **kwargs):
    ...
    def send(self, to, *args, **kwargs):
        ...
        super(CustomBaseEmail, self).attach_alternative(html_content,
                                                        'text/html')
        super(CustomBaseEmail, self).send(*args, **kwargs)


class CustomActivationEmail(CustomBaseEmail):
    template_name = 'users/activation/activation_email.html'
    template_name_txt = 'users/activation/activation_email.txt'
...
class CustomConfirmationEmail(CustomBaseEmail):
    template_name = 'users/confirmation/confirmation_email.html'
    template_name_txt = 'users/confirmation/confirmation_email.txt'
    email_subject = f'{SITE_NAME} - Account Activated'


class CustomPasswordResetEmail(CustomBaseEmail):
    template_name = 'users/password_reset/password_reset_email.html'
    template_name_txt = 'users/password_reset/password_reset_email.txt'
    ...
    def get_context_data(self, **kwargs):
        ...
        uid, token = get_uid_and_token(context)
        url = djoser_settings.PASSWORD_RESET_CONFIRM_URL.format(
            uid=uid, token=token
        )
...
class CustomPasswordChangedConfirmationEmail(CustomBaseEmail):
    template_name = 'users/password_changed/password_changed_email.html'
    template_name_txt = 'users/password_changed/password_changed_email.txt'
    email_subject = f'{SITE_NAME} - Password Changed'


class CustomUsernameChangedConfirmationEmail(CustomBaseEmail):
    template_name = 'users/username_changed/username_changed_email.html'
    template_name_txt = 'users/username_changed/username_changed_email.txt'
    email_subject = f'{SITE_NAME} - Username Changed'


class CustomUsernameResetEmail(CustomBaseEmail):
    template_name = 'users/username_reset/username_reset_email.html'
    template_name_txt = 'users/username_reset/username_reset_email.txt'
    ...
    def get_context_data(self):
        ...
        uid, token = get_uid_and_token(context)
        url = djoser_settings.USERNAME_RESET_CONFIRM_URL.format(
            uid=uid, token=token
        )
        context['reset_url'] = f'{URL}/{url}'
        return context
